 ...
 *      front of it. Covers outbound uploads and, since migration 047,
 *      inbound media mirrored into the same bucket.
 *   3. A synthesised `whatsapp-<kind>-<timestamp>.<ext>`. The extension comes
 *      from `media_type` when the row has one (migration 047 — the webhook
...
export interface MediaFilenameInput {
  ...
  content_text?: string;
  media_url?: string;
  /** Meta's MIME type for inbound media (migration 047); null on older rows. */
