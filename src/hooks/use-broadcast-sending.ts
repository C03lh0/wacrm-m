...
interface BroadcastPayload {
  ...
  /**
   * When set, the broadcast is persisted as `status: 'scheduled'`
   * with recipient rows pre-created (status 'pending') instead of
   * sending immediately — the cron dispatcher
   * (deliverScheduledBroadcast, src/app/api/broadcasts/cron)
   * resolves each recipient's variables fresh from `variables` +
   * live contact data and sends once `scheduledAt` is due.
   */
  scheduledAt?: string;
}
...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
    ...
    // Exclude contacts who opted out of broadcasts (migration 039) —
    // same compliance backstop enforced by the public API's
    // createBroadcast, so the dashboard wizard and the API agree.
  ...
  async function createAndSendBroadcast(payload: BroadcastPayload): Promise<string> {
    ...
    try {
      ...
      // A scheduled broadcast stops here — recipient rows exist
      // (status 'pending'), but sending is deferred to the cron
      // dispatcher (deliverScheduledBroadcast, src/app/api/broadcasts/
      // cron) once scheduled_at is due. Variables are re-resolved
      // from live contact data at that point rather than persisted
      // now — see template-variables.ts.
