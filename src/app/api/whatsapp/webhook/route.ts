...
import {
  ...
  ingestParsedMessage,
  lookupInternalIdByProviderMessageId,
} from '@/lib/whatsapp/inbound-message-pipeline'
    ...
    for (const change of entry.changes) {
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        await processMessage(
          ...
          // (migration 046), Meta's equivalent of an Evolution instance id.
          config.id,
          decryptedAccessToken,
          // Default ON: the column is NOT NULL DEFAULT TRUE, but a row
          // read before migration 047 lands would have it undefined,
...
async function processMessage(
  ...
  connectionId: string,
  accessToken: string,
  // Per-account opt-out for the inbound-media mirror (migration 047).
  ...
  // below — so a conversation first opened by a reaction still fires the
  // event, and a subscriber always sees the thread open before its first
  // message.received. Shared with the Evolution webhook (see
  // src/lib/whatsapp/inbound-message-pipeline.ts).
  ...
  // detected via migration 046's dedup index and skipped rather than
  // double-inserted.
  await ingestParsedMessage(supabaseAdmin(), resolved, {
    ...
    content: { contentType, contentText, mediaUrl, mediaType, interactiveReplyId },
...
// src/lib/whatsapp/inbound-message-pipeline.ts (imported above as
// resolveContactAndConversation) so the Evolution webhook can reuse
// the exact same contact/conversation resolution logic.
